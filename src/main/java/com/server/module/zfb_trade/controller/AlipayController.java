...
import com.server.module.store.OrderForm;
...
//		}
	...
	public void balanceAliPay(@RequestParam String payCode, HttpServletRequest request,
		...
		if(memberOrder !=null && !memberOrder.getState().equals(PayStateEnum.PAY_SUCCESS.getState().toString())){
			...
			log.info("alipayClient"+JSON.toJSONString(alipayClient));
	...
	public void huafaAppPay(OrderForm orderForm,HttpServletRequest request,HttpServletResponse response) throws IOException {
		log.info("<AlipayController--appPay--start>");

		log.info("<AlipayController--appPay--start>");
		try {
			Map<String, Object> id = storeOrderService.findSomeMessByOrderId(orderForm);

			String payCode = (String) id.get("payCode");
			String product = (String) id.get("product");
			double nowprice=(double) id.get("nowprice");
			Integer companyId=1;
			log.info("nowprice"+nowprice);
			//redisClient.set("Price_"+customerId, id.get("nowprice").toString(),60*5);

			//http://yms.youshuidaojia.com/aliUser/forward
			//alipayRequest.setReturnUrl("http://yms.youshuidaojia.com/aliUser/balanceForward");
			...
			//AlipayConfig alipayConfig = alipayConfigFactory.getAlipayConfig(companyId);
			//http://yms.youshuidaojia.com/aliUser/forward
			alipayRequest.setReturnUrl("http://free-tcp.svipss.top:15238/aliUser/forward");
			alipayRequest.setNotifyUrl("http://yms.youshuidaojia.com/aliInfo/appNotify");// 在公共参数中设置回跳和通知地址
			alipayRequest.setBizContent("{"
